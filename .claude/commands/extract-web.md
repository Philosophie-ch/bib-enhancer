---
description: Extract bibliographic references from a URL using RawWebText workflow
argument-hint: <url> [output-path]
---

Extract bibliographic references from the provided URL using the RawWebText manual workflow.

## IMPORTANT: Check Registry First!

**Before starting extraction, check if this URL or domain is in the registry:**
`philoch_bib_enhancer/adapters/raw_web_text/extractors/registry.json`
...
## RawWebText Manual Workflow

### 1. Fetch the Page
Use `fetch_url_text()` from `philoch_bib_enhancer.adapters.raw_web_text.web_scraper`
...
### 3. Create RawWebTextBibitem Objects
For each reference, create a `RawWebTextBibitem` object with all extractable fields:
...
- `authors` - List of `RawWebTextAuthor` objects with `given` and `family` names
- `editors` - List of `RawWebTextAuthor` objects (for edited volumes)
...
### 4. Convert to CSV
Use `process_raw_bibitems()` from `philoch_bib_enhancer.cli.manual_raw_web_text_to_csv` to convert to CSV format.

### Example Reference
See `examples/manual_workflow_example.py` for `RawWebTextBibitem` structure examples.
...
**Steps:**
1. Check `philoch_bib_enhancer/adapters/raw_web_text/extractors/registry.json` for this URL/domain
2. If found in registry, follow the documented extraction method
3. If not in registry, use the standard RawWebText workflow above
4. Extract all bibliographic references
5. Create `RawWebTextBibitem` objects
