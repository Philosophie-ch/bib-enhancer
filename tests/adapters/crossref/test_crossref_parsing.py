...
def test_crossref_article_model_validation(raw_crossref_articles: list[dict[str, object]]) -> None:
...
def test_crossref_article_conversion(raw_crossref_articles: list[dict[str, object]]) -> None:

    raw_articles = raw_crossref_articles

    for raw_article in raw_articles:
        cr_article = CrossrefArticle.model_validate(raw_article)
        bib_item = _convert_crossref_article_to_bibitem(cr_article)
        assert bib_item is not None, "BibItem conversion failed"


def test_crossref_article_conversion_to_bibitem(raw_crossref_article: tuple[list[str], dict[str, object]]) -> None:
